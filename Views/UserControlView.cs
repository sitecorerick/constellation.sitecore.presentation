		...
		where TModel : class
	{
		...
		private Item GetSingleItemFromDataSource()
		{
			...
			if (dataSource.Length > 0)
			{
				...
				{
					Assert.IsNotNull(item, "Cannot resolve a relative data source path when the current item is null. Path is " + dataSource);
			...
			if (item == null)
			{
				Tracer.Warning("Rendering \"" + this.GetType().FullName + "\" will not be shown as the DataSource \"" + dataSource + "\" was not found.");
			}
